...
public class KieServicesTest extends AbstractKieServicesBaseTest {
    ...
    public void testQuery() {
        ...
        userTaskService.complete(taskId, "john", null);

        SqlQueryDefinition query = new SqlQueryDefinition("getAllProcessInstances", "jdbc/testDS1");
        query.setExpression("select * from processinstancelog");
        queryService.registerQuery(query);
        Collection<ProcessInstanceDesc> instances = queryService.query("getAllProcessInstances", ProcessInstanceQueryMapper.get(), new QueryFilter(0, 100));
        logger.info("instances.size() = " + instances.size());
        for (ProcessInstanceDesc pid : instances) {
            System.out.println("ProcessName = " + pid.getProcessName());
